...
(function () {
  ...
  var KB = {
    ...
    // ข้อพิพาทแนวเขต — คำถามที่ลูกค้าถามบ่อยและเป็นงานหลักของเราโดยตรง จึงตอบเองไม่ส่งให้ AI
    // ⚠️ ห้ามให้คำแนะนำทางกฎหมาย (ฟ้องได้ไหม · ครอบครองปรปักษ์กี่ปี · เรียกค่าเสียหายเท่าไหร่)
    //    ตอบได้แค่ "ยืนยันแนวเขตด้วยการรังวัดก่อน" ซึ่งเป็นสิ่งที่เราทำจริง แล้วส่งต่อให้ทนาย/ทีมงาน
    dispute: 'เรื่องแนวเขตกับข้างเคียง เริ่มจากยืนยันตำแหน่งหลักเขตจริงก่อนเสมอครับ ตราบใดที่ยังคุยกันจากแนวรั้วเดิมหรือความจำ จะไม่มีฝ่ายไหนพิสูจน์ได้ว่าใครล้ำใคร\n' +
      '1) ตรวจโฉนดและระวางแผนที่ของแปลงตัวเองก่อน ว่าเนื้อที่และรูปแปลงตรงกับที่ใช้อยู่จริงไหม\n' +
      '2) รังวัดสอบเขต เพื่อหาตำแหน่งหลักเขตจริงในสนาม แล้วเทียบกับแนวรั้ว/สิ่งปลูกสร้างที่มีอยู่\n' +
      '3) วันรังวัด เจ้าของแปลงข้างเคียงจะถูกแจ้งให้มาระวังแนวเขตด้วย ผลรังวัดจึงใช้อ้างอิงกับอีกฝ่ายได้ ไม่ใช่การวัดเองฝ่ายเดียว\n' +
      '4) ได้รูปแผนที่และตัวเลขที่ยืนยันแล้ว ค่อยใช้เป็นหลักฐานคุยกับข้างเคียง\n\n' +
      'ถ้าคุยแล้วยังตกลงกันไม่ได้ ขั้นต่อไปเป็นเรื่องกฎหมาย ซึ่งน้องกับทีมงานไม่ให้คำปรึกษาครับ — แต่รูปแผนที่จากการรังวัดคือหลักฐานตั้งต้นที่ทนายใช้\n' +
      'หมายเหตุ: ช่างรังวัดเอกชนรับงานได้เฉพาะที่ดินที่มีโฉนดแล้ว',
  ...
  function route(text) {
    ...
    // ลูกค้าวางลิงก์ประกาศ = อยากให้ตรวจทรัพย์นั้น → ส่งไปหน้าทรัพย์หน่วยงานพร้อมลิงก์ (P1 · 2026-09-17)
    // ⚠️ ไม่เปิดลิงก์ ไม่ส่งลิงก์ให้ AI และไม่ทำลิงก์ของลูกค้าเป็นปุ่มกด (กติกาข้อ 2 ใน agencies.js)
    //    ลิงก์ในคำตอบชี้ไปหน้าของเราเองเท่านั้น
    var pasted = /https?:\/\/[^\s<>"']+/i.exec(String(text));
    if (pasted) {
      var ag = window.NJAgency ? NJAgency.detect(pasted[0]) : null;
      var from = ag && ag.ok && ag.kind === 'agency' ? 'ลิงก์นี้มาจาก <b>' + esc(ag.label) + '</b> ' : 'ได้รับลิงก์แล้วครับ ';
      return Promise.resolve({
        html: from + '— น้องเปิดดูเนื้อหาในลิงก์เองไม่ได้ แต่ส่งให้ทีมช่างรังวัดตรวจเอกสาร แนวเขต และทางเข้าออกก่อนซื้อได้ครับ ' +
          link('agency.html?url=' + encodeURIComponent(pasted[0].slice(0, 300)), 'ส่งลิงก์นี้ให้ทีมตรวจ →'),
        chips: ['คุยกับเจ้าหน้าที่']
      });
    }
    ...
    // ข้อพิพาทแนวเขต / เพื่อนบ้านรุกล้ำ — ต้องมาก่อนโหมดค้นแปลง ไม่งั้น "ข้างบ้าน" กับ "ที่ดิน" จะลากเข้าไปค้นประกาศ
    // ⚠️ คำว่า "เพื่อนบ้าน/ข้างบ้าน" เดี่ยวๆ กว้างเกินไป ("ที่ดินข้างบ้านขายไหม" คือคนอยากซื้อ ไม่ใช่มีเรื่อง)
    //    จึงต้องเจอคำที่บ่งว่ามีปัญหาจริง หรือเจอคู่ "คนข้างเคียง + คำเรื่องแนวเขต" พร้อมกัน
    if (has(q, ['รุกล้ำ', 'ล้ำแนว', 'ล้ำเขต', 'ล้ำที่', 'ล้ำเข้ามา', 'ทับซ้อน', 'พิพาท', 'แย่งที่',
               'เขตไม่ตรง', 'แนวเขตไม่ตรง', 'หลักเขตหาย', 'หมุดหาย', 'ย้ายหลักเขต', 'ปักหลักเขต',
               'ชี้แนวเขต', 'เนื้อที่ขาด', 'เนื้อที่หาย', 'ที่ดินหาย']) ||
        (has(q, ['เพื่อนบ้าน', 'ข้างบ้าน', 'แปลงข้างเคียง', 'ที่ดินข้างเคียง', 'คนข้างๆ']) &&
         has(q, ['แนวเขต', 'หลักเขต', 'แนวรั้ว', 'รั้ว', 'เขตที่ดิน', 'หมุด'])))
      return Promise.resolve({ html: nl2br(KB.dispute) + '<br>' + link('guides.html#survey', 'ทำไมต้องรังวัดสอบเขต →'), chips: ['ค่ารังวัดสอบเขต 2 ไร่ เท่าไหร่', 'ขั้นตอนยื่นรังวัด', 'คุยกับเจ้าหน้าที่'] });
    // ถามความหมาย/วิธีตรวจผังสี (ไม่ใช่ค้นแปลงตามสี) → คู่มือผังสี · ห้ามตอบว่าแปลงไหนสร้างอะไรได้ (P3 · 2026-09-17)
    // ⚠️ ต้องมีคำถามกำกับ — "ที่ดินผังเหลือง" เฉยๆ คือโจทย์ค้นแปลง ต้องไปโหมดค้นตามเดิม
    if (has(q, ['ผังสี', 'ผังเมือง', 'สีเหลือง', 'สีส้ม', 'สีแดง', 'สีม่วง', 'สีเขียว', 'ผังเหลือง', 'ผังเขียว', 'ผังแดง', 'ผังส้ม', 'ผังม่วง']) &&
        (has(q, ['คืออะไร', 'หมายถึง', 'แปลว่า', 'สร้างอะไรได้', 'ทำอะไรได้', 'สร้างได้ไหม', 'ทำได้ไหม', 'ตรวจผัง', 'เช็คผัง', 'เช็กผัง', 'ดูผัง', 'หาผัง', 'ตรวจสอบผัง']) ||
         /(สร้าง|ทำ|ตั้ง|เปิด|ปลูก).{0,24}ได้(ไหม|มั้ย|หรือไม่|หรือเปล่า|รึเปล่า)/.test(q)))
      return Promise.resolve({ html: 'สีในผังเมืองรวมบอกประเภทการใช้ประโยชน์ที่ดิน เช่น เหลือง = ที่อยู่อาศัยหนาแน่นน้อย เขียว = ชนบทและเกษตรกรรม ' +
        'แต่ข้อห้ามจริงต่างกันตามผังแต่ละฉบับ น้องจึงตอบแทนไม่ได้ว่าแปลงไหนสร้างอะไรได้ครับ ' +
        link('tools.html#zoning', 'ดูความหมายของแต่ละสี + ลิงก์ตรวจผังของหน่วยงาน →') + '<br>' +
        'หรือให้ทีมตรวจผังเมืองของแปลงให้ ' + link('verify.html?topic=zoning#form', 'ส่งแปลงให้ทีมตรวจ →'), chips: ['คุยกับเจ้าหน้าที่'] });
