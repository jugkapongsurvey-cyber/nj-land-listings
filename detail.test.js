...
{
  // ⭐ หัวข้ออ้างจังหวัดได้เฉพาะเมื่อทุกแปลงที่แสดงอยู่ในจังหวัดนั้น (เคยขึ้น "ในสระบุรี" ทั้งที่มีแปลงอยุธยา/สุพรรณบุรีปน)
  const m = land.match(/function relatedTitle\(prov,shown\)\{[\s\S]*?\n  \}/);
  ok('มีตัวตั้งหัวข้อแปลงใกล้เคียง (relatedTitle)', !!m);
  ok('หัวข้อใช้ relatedTitle ไม่ต่อชื่อจังหวัดเอง', /esc\(relatedTitle\(prov,similar\)\)/.test(land) && !/'แปลงใกล้เคียง'\+\(prov\?/.test(land));
  if (m) {
    const relatedTitle = new Function(m[0] + '; return relatedTitle;')();
    const P = (p) => ({ land: { province: p } });
    ok('⭐ ทุกแปลงอยู่จังหวัดเดียวกัน = อ้างจังหวัดได้',
       relatedTitle('สระบุรี', [P('สระบุรี'), P('สระบุรี')]) === 'แปลงใกล้เคียง ในสระบุรี');
    ok('⭐ มีแปลงจังหวัดอื่นปน = ไม่อ้างจังหวัด',
       relatedTitle('สระบุรี', [P('สระบุรี'), P('พระนครศรีอยุธยา'), P('สุพรรณบุรี')]) === 'แปลงอื่นที่น่าสนใจ');
    ok('ไม่มีแปลงจังหวัดเดียวกันเลย = ไม่อ้างจังหวัด',
       relatedTitle('สระบุรี', [P('นครปฐม')]) === 'แปลงอื่นที่น่าสนใจ');
    ok('แปลงนี้ไม่ได้กรอกจังหวัด = ไม่อ้างจังหวัด',
       relatedTitle('', [P(''), P('')]) === 'แปลงอื่นที่น่าสนใจ');
    ok('แปลงที่แสดงไม่ได้กรอกจังหวัด ≠ อยู่จังหวัดเดียวกัน',
       relatedTitle('สระบุรี', [P('สระบุรี'), {}]) === 'แปลงอื่นที่น่าสนใจ');
  }
}
