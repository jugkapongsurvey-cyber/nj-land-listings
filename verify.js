...
function setupContactLinks() {
  [['vf-line', 'line'], ['vf-done-line', 'line'], ['vf-bar-line', 'line'],
   ...
   ['vf-tel', 'tel'], ['vf-done-tel', 'tel'], ['vf-bar-tel', 'tel'],
   ['vf-tel2', 'tel2'], ['vf-done-tel2', 'tel2'], ['vf-bar-tel2', 'tel2']].forEach(function (pair) {
...
function showErr(msg, field) {
  if (!vfErrs && window.NJForm) vfErrs = NJForm.errors($('verify-form'));
  if (vfErrs) { vfErrs.clear(); if (msg && field) vfErrs.set(field, msg); }
  var box = $('vf-err');
  if (!msg) { box.hidden = true; box.textContent = ''; return; }
  box.textContent = msg;
  box.hidden = false;
  // พาไปที่ช่องที่ผิดเลย ดีกว่าเลื่อนไปกล่องสรุปแล้วให้ผู้ใช้ไล่หาเองว่าช่องไหน
  if (vfErrs && field && vfErrs.focusFirst()) return;
...
var PREFILL_TOPICS = {
  ...
  area: 'ขอให้รังวัดสอบเขตและยืนยันเนื้อที่จริงของแปลงนี้ (วัดจากภาพแผนที่ไว้แล้ว ต้องการตัวเลขที่ใช้อ้างอิงได้)'
};
