  ...
  fetch(NJ_API_BASE + '/api/public/listings')
    ...
    .then(function (data) {
      ...
      if (!listings.length) {
        ...
        // ทางออกหลักคือ "ฝากหาที่ดิน" เพราะคนที่มายืนอยู่หน้ารายการแปลงคือ **ผู้ซื้อ**
        // ยื่นให้แต่ปุ่มฝากขาย = ตอบไม่ตรงกับสิ่งที่เขามาหา แล้วเขาก็ปิดหน้าไป
        // (ปุ่มฝากขายยังอยู่ เพราะคนสนใจที่ดินจำนวนหนึ่งเป็นเจ้าของที่ดินเองด้วย แต่ไม่ใช่ปุ่มหลัก)
        grid.innerHTML =
          '<div class="lc-empty">' +
            '<div class="lc-empty-icon">🌱</div>' +
            // ⚠️ ข้อความชุดเดียวกันมีอีกที่ใน listingcard.js — แก้ที่เดียวคืออีกหน้าไม่ตรงกัน
            '<div class="lc-empty-title">ยังไม่มีแปลงที่เปิดประกาศอยู่ตอนนี้</div>' +
            '<div class="lc-empty-text">ทุกแปลงจะขึ้นเว็บได้ก็ต่อเมื่อเจ้าของที่ดินยินยอมให้เผยแพร่แล้ว และเราจะระบุไว้บนการ์ดเสมอว่าแปลงนั้นตรวจถึงระดับไหนและตรวจเมื่อไหร่ — ' +
              'ระหว่างนี้บอกโจทย์ที่คุณกำลังหาไว้ได้เลย ทีมช่างรังวัดของเราจะหาแปลงที่ตรงเงื่อนไขให้</div>' +
            '<div class="lc-empty-btns">' +
              '<a class="lc-btn lc-btn-primary" href="wanted.html">🔎 ฝากหาที่ดินฟรี — บอกโจทย์ไว้ ให้เราหาให้ →</a>' +
              '<a class="lc-btn lc-btn-line" href="' + esc(LINE_OA_URL) + '" target="_blank" rel="noopener" data-track="line">💬 แจ้งเตือนแปลงใหม่ทางไลน์</a>' +
              '<a class="lc-btn" href="consign.html">มีที่ดินอยากขาย?</a>' +
            '</div>' +
          '</div>';
        return;
      }
