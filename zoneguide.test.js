...
// ⚠️ เจ้าของกิจการยกเลิกหนังสือขออนุญาตแสดงผังเมืองทั้งสองฉบับเมื่อ 22 ก.ย. 69 แล้วใช้ทางลิงก์ออกถาวร
//    ข้อความนี้จึงต้องอยู่ในหน้า ไม่ใช่แค่ในคอมเมนต์ — ผู้ใช้ต้องอ่านออกว่าเป็นการตัดสินใจ ไม่ใช่ของที่ยังทำไม่เสร็จ
ok('บอกในหน้าว่าเว็บนี้ไม่แสดงภาพผังเมืองเอง', /ไม่ได้แสดงภาพผังเมืองเอง/.test(G.NO_MAP) && h.indexOf(G.NO_MAP.slice(0, 40)) >= 0);
ok('เหตุผลพูดถึงทั้งหนังสืออนุญาตและความใหม่ของข้อมูล', /หนังสืออนุญาต/.test(G.NO_MAP) && /ไม่ใช่ฉบับล่าสุด/.test(G.NO_MAP));
ok('ไม่มีภาพผังของหน่วยงานฝังอยู่ในไฟล์คู่มือ',
   !/<iframe|<img|cityplan|dptgis|MapServer|WMS/i.test(read('zoneguide.js').replace(/\/\*[\s\S]*?\*\//g, '').replace(/^[ 	]*\/\/.*$/gm, '')));
